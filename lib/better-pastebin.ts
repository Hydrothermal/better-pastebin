...
module.exports = pbapi = {
  setDevKey: function(devkey: string) {
    pbapi.devkey = devkey;
  },

  setLoginCookie: function(cookie: string) {
...
*/
    try {
      ...
      if (response.body === "Bad API request, invalid api_dev_key") {
        if (cb) {
          cb(false, new Error("Invalid devkey"));
        } else {
          pbapi.userkey = response.body;
          done();
        }
      }
    }
